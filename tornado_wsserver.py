from database.connect_db import REDIS_HOST, REDIS_PASSWORD, REDIS_TABLENAME
import redis
import json
...
pool = redis.ConnectionPool(host=REDIS_HOST, password=REDIS_PASSWORD,decode_responses=True)
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                pass
            redis_conn.hset(REDIS_TABLENAME, "websocket_end",
                            datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
            redis_conn.hset(REDIS_TABLENAME, "websocket_runcount", str(runcount))
            redis_conn.hset(REDIS_TABLENAME, "websocket_failcount", str(failcount))
            redis_conn.hset(REDIS_TABLENAME, "websocket_status", "执行成功")
